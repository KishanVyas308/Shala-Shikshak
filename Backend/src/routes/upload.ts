...
import { compressPDF, checkGhostscriptAvailability } from '../utils/pdfCompressor';
...
router.post('/pdf', authenticateToken, upload.single('pdf'), async (req, res) => {
  try {
    ...
    try {
      ...
      const compressedFilePath = path.join(path.dirname(tempFile.filePath), compressedFileName);
      
      const compressionResult = await compressPDF(tempFile.filePath, compressedFilePath);
